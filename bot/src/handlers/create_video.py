...
async def get_video_for_video_from_video(message: Message):
    await message.answer("Вижу! Видео загружается. Пожалуйста, подождите пару секунд. Пока что можете поискать у себя аудиофайлы под это видео, его нужно будет скинуть следующим этапом")
    chat_id = message.chat.id
    if chat_id not in global_state:
...
async def create_video_from_video_for_youtube(message: Message):
    ...
    if chat_id in global_state:
        ...
        await message.answer_video(video=video_input, reply_markup=youtube)
